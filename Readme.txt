...
8th May 2013 - Version 2.4.2
  ...
  2.  Moved 'float: left' to styles.css for Henrik Thorn - CONTRIB-4198.
  3.  Realised that Tablets have more space, so allow two columns even when two or more are set.
  4.  Moved all 'cnconfig.php' default functionalty to 'Site Administration -> Plugins -> Course formats -> Columns'
      so that defaults can be changed by the administrator from within Moodle without resorting to code changes.
  5.  Added capability 'format/columns:changecolumns' to editing teachers and managers such that site administrators can choose to
